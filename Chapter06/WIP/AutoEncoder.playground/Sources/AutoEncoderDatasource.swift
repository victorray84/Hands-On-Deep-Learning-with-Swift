...
public class AutoEncoderDataSource : NSObject, MPSCNNConvolutionDataSource{
    
    public static let FolderName = "autoencoder_weights"
    ...
    public func copy(with zone: NSZone? = nil) -> Any {
        let copy = AutoEncoderDataSource(
            name: self.name,
...
extension AutoEncoderDataSource{
    ...
    private func generateRandomWeights() -> Data?{
        ...
        let minValue = -1.0 * sqrt(2.0 / Float(self.outputFeatureChannels))
        let maxValue = 1.0 * sqrt(2.0 / Float(self.outputFeatureChannels))
        
        for index in 0..<count{
            randomWeights[index] = Float32.random(in: minValue...maxValue)
        }
