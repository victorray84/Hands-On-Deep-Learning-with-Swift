...
let untrainedNetworkForInference = Network(
    withCommandQueue: commandQueue, mode: .inference)
untrainedNetworkForInference.predict(
    x: sample!.image,
    completationHandler: { (result) in
...
let trainedNetworkForInference = Network(
    withCommandQueue: commandQueue, mode: .inference)

trainedNetworkForInference.predict(
    x: sample!.image,
    completationHandler: { (result) in
