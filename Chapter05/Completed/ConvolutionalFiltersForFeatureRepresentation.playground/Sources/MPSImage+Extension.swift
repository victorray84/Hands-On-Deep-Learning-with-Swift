...
public extension MPSImage{
    ...
    @nonobjc public func toFloatArray() -> [Float]?{
        ...
        /*
         If featureChannels<=4 and numberOfImages=1 (i.e. only one slice is needed to
         represent the image), the underlying metal texture type is chosen to be
         MTLTextureType.type2D rather than MTLTextureType.type2DArray as explained above.
         */
        let channels = self.textureType == MTLTextureType.type2D ? self.featureChannels : numberOfSlices * 4
        
        let numberOfComponents = self.textureType == MTLTextureType.type2D ? self.featureChannels : numberOfSlices * 4
        let stride = self.width * self.height * numberOfComponents
        
        let count = self.width * self.height * channels * self.numberOfImages
        var outputUInt16 = [UInt16](repeating: 0, count: count)
        
        let bytesPerRow = self.width * numberOfComponents * MemoryLayout<UInt16>.size
